...
public class NetCommander {
    ...
    private static Proxy getProxy(Proxy.Type proxyType) {
        ...
        if (QProperties.get().getProperty("proxy", "hostname") != null && QProperties.get().getProperty("proxy", "port") != null) {
            QLog.l().logger().trace("Proxy.Type." + proxyType + ": " + QProperties.get().getProperty("proxy", "hostname").getValue() + ":" + QProperties.get().getProperty("proxy", "port").getValueAsInt());
    ...
    synchronized public static String sendRpc(INetProperty netProperty, JsonRPC20 jsonRpc) throws QException {
        ...
        try {
            if (QConfig.cfg().getHttpRequestType() != null && !(jsonRpc.getMethod().startsWith("#") || "empty".equalsIgnoreCase(jsonRpc.getMethod()))) {
                ...
                final Socket socket = proxy == null ? new Socket() : new Socket(proxy);
                try {
    ...
    private static String sendPost(INetProperty netProperty, String outputData, JsonRPC20 jsonRpc) throws Exception {
        ...
        final HttpURLConnection con = (HttpURLConnection) (proxy == null ? obj.openConnection() : obj.openConnection(proxy));
